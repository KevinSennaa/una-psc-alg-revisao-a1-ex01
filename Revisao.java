...
public class Revisao {
    public static void main(String[] args) {
        ...
        while(stop == false){
            ...
            while(contador <= 3){
                System.out.println("Seu cartão de vacinção está em dia? (SIM/NÂO)");
                cartãoVAC = scanner.next().toUpperCase();
                if(cartãoVAC.equals("NAO") ){
                    porcentagemInfectado += 10;
                }
                if(cartãoVAC.equals("SIM") || cartãoVAC.equals("NAO")){
                    break;
                }
                else if(contador >= 3 ){
            ...
            System.out.println("Cartão de vacina em dia: " + cartãoVAC);
